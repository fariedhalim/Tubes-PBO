...
play_bt = pygame.transform.scale(pygame.image.load("assets/images/play_bt.png").convert_alpha(), (300, 75))
play_bt_rect = play_bt.get_rect(midbottom = (500, 350))
...
char2 = Player(2, 800, 310, True, WIZARD_DATA, wizard_sheet, WIZARD_ANIMATION_STEPS, magic_fx)
...
#function for drawing energy bars
...
while run:
  ...
  if play:
    ...
    #show player stats
    health_bar(char1._hp, 20, 20)
    health_bar(char2._hp, 580, 20)
    ...
    if round_over == False:
      ...
      draw_text('Game Over', gameover_font, RED, 375, 150)
      if p1_win:
        draw_text('Player 1 Win', gameover_font, GREY, 360, 200)
      elif p2_win:
        draw_text('Player 2 Win', gameover_font, GREY, 360, 200)
    ...
    draw_bg()
    draw_text("PAZ", title_font, TITLE, 425, 95)
    screen.blit(play_bt, play_bt_rect)
    mouse_pos = pygame.mouse.get_pos()
    if play_bt_rect.collidepoint(mouse_pos):
      for event in pygame.event.get():
